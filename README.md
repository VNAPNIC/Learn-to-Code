# Learn to Code - for Free 


[![Gitter](https://badges.gitter.im/buihainam-com/community.svg)](https://gitter.im/buihainam-com/community?utm_source=badge&utm_medium=badge&utm_campaign=pr-badge&utm_content=body_badge)

# Lessons

### Java

- [Ngôn ngữ Java là gì](https://buihainam.com/java/ngon-ngu-java-la-gi/)
- [OOP Trong Java](https://buihainam.com/java/cac-tinh-chat-huong-doi-tuong-cua-java/)

### JUnit

- [JUnit là gì](https://buihainam.com/junit/junit-la-gi-vi-du-junit-voi-eclipse-maven/)
- [Assertions là gì](https://buihainam.com/junit/assertions-la-gi-code-vi-du-junit-voi-assertions/)
- [Test runner và Test suite](https://buihainam.com/junit/test-runner-va-test-suite-tao-bo-test-voi-junit/)
- [Thứ tự chạy các test case trong JUnit](https://buihainam.com/junit/thu-tu-chay-cac-test-case-trong-junit-vi-du/)
- [JUnit Expected Exceptions](https://buihainam.com/junit/junit-expected-exceptions-code-vi-du-test-exception-voi-junit/)
- [JUnit với Matchers và assertThat](https://buihainam.com/junit/vi-du-junit-voi-matchers-va-assertthat/)
- [JUnit Ignoring và Timeout với test case](https://buihainam.com/junit/vi-du-junit-ignoring-tests-va-timeout-voi-test-case/)

### Dart

- [Bắt đầu](https://buihainam.com/dart/dart_environment_settings/)
- [Cơ bản](https://buihainam.com/dart/dart_core/)
- [Dart Async](https://buihainam.com/dart_async_intro/)

### Kotlin

- [Kotlin Basic](https://buihainam.com/kotlin/kotlin_basic/)
- [Kotlin Class và kế thừa](https://buihainam.com/kotlin/kotlin_class_inheritance/)
- [Kotlin Property và field](https://buihainam.com/kotlin/kotlin_property_field/)
- [Kotlin Funtion](https://buihainam.com/kotlin/kotlin_function/)
- [Kotlin Higher order funtion](https://buihainam.com/kotlin/kotlin_higher_order_function/)
- [Kotlin Inline function](https://buihainam.com/kotlin/kotlin_inline_function/)
- [Kotlin Inteface](https://buihainam.com/kotlin/kotlin_inteface/)
- [Kotlin Data class](https://buihainam.com/kotlin/kotlin_data_class/)
- [Kotlin Nested class](https://buihainam.com/kotlin/kotlin_nested_class/)
- [Kotlin Visibility Modifiers](https://buihainam.com/kotlin/kotlin_visibility_modifiers/)
- [Kotlin Lambda](https://buihainam.com/kotlin/kotlin_lambda/)
- [Kotlin Collection](https://buihainam.com/kotlin/kotlin_collection/)
- [Kotlin Enum Classes](https://buihainam.com/kotlin/kotlin_enum_classes/)
- [Kotlin Overloading operator](https://buihainam.com/kotlin/kotlin_overloading_operator/)
- [Kotlin Range](https://buihainam.com/kotlin/kotlin_range/)
- [Kotlin Idioms](https://buihainam.com/kotlin/kotlin_idioms/)
- [Kotlin Exception](https://buihainam.com/kotlin/kotlin_exception/)
- [Kotlin Delegation](https://buihainam.com/kotlin/kotlin_delegation/)
- [Kotlin Delegated property](https://buihainam.com/kotlin/kotlin_delegated_property/)
- [Gọi Kotlin từ Java](https://buihainam.com/kotlin/call_kotlin_from_java/)
- [Gọi Java code từ Kotlin](https://buihainam.com/kotlin/call_java_from_kotlin/)


### Flutter

- [9 lý do tại sao nên chọn Flutter](https://buihainam.com/flutter/flutter_introduction/)
- [Flutter ScreenUtil](https://buihainam.com/flutter/flutter_screenutil/)
